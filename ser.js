...
var fs = require('fs');
var url = require('url');
var link = http.createServer(function(req, res) {
	var basePath = '.',
	json = url.parse(req.url, true),
	file = decodeURI(json.pathname) || '/index.html',
	_get = json.query;
	if (_get && _get.jsonp) { //jsonp数据请求
		_ser.get(res, file, _get.jsonp);
	} else {
		_ser.get(res, file);
	}
}).listen(8080);
...
var _ser = {
	get: function(res, file, jsonp) {
		...
		fs.readFile(basedir + file, function(err, data) {
			if (err) {
				...
				if (jsonp) 
					data = '(function(){' + data + ' window["' + jsonp +'"](json);})();'	
				res.writeHead(200, {
	...
	extname: function(file) {
		...
		return index < 0 ? ".html" : file.substring(index);
	}
};
