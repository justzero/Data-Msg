(function($) {
	...
	cache.nosyn.RequestDiscountInfo = true;
	modules.RequestDiscountInfo = function(callback) {
		...
		var json = {
			...
			success: function(json) {
				cache.data.discountInfo = json;
				callback.success();
			},
			error: function() {
				cache.data.discountInfo = {
					code: '110000'
				};
				callback.success();
			}
		};
		if (cache.conf.test === 2) {
            json.params = {
                'mmurl': consts.mmUrl
            };
        }
		if (cache.conf.test === 1 && cache.localConf.filename) {
            json.params = {
